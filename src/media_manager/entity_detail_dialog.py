...
class EntityDetailDialog(QDialog):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._current_entity: PersonDetails | CompanyDetails | None = None
    ...
    def _create_right_panel(self) -> QWidget:
        ...
        self._credits_table.setSelectionBehavior(
    ...
    def show_person(self, person_id: int) -> None:
        ...
        for row, entry in enumerate(person.filmography):
            ...
            # Role
            self._credits_table.setItem(
                row, 2, QTableWidgetItem(entry.role.capitalize())
            )
    ...
    def show_company(self, company_id: int) -> None:
        ...
        for row, production in enumerate(company.productions):
            ...
            # Type
            self._credits_table.setItem(
                row, 2, QTableWidgetItem(production.media_type.capitalize())
            )

            # Rating
            if production.rating:
                rating_text = f"{production.rating:.1f}/10"
                stars = "★" * int(production.rating / 2) + "☆" * (
                    5 - int(production.rating / 2)
                )
                rating_text += f" {stars}"
            else:
                rating_text = "-"
